...
class Wall(elements.Mob):
    """This is an impassible wall. Characters should not pass through it, nor see through it."""
    def init(self):
        super(Wall,self).init()
        self.values.append("name")
        self.radius = 5
        self.radius_sq = self.radius**2
        self.values.extend(['radius'])
        self.template = "LocalWall"
...
class LocalClient(local.Mob):
    ...
    def init(self):
        ...
        self.add_property(local.NetworkedProperty("fx",predicted=True))
        self.add_property(local.NetworkedProperty("fy",predicted=True))
    def draw(o,s):
        s.blit(f.render(o.name,1,[255,255,255]),[o.x,o.y])
        if not hasattr(o,'fx'):
            return
        if o.fx:
            s.blit(f.render("-",1,[255,255,255]),[o.x+o.fx*6,o.y])
        elif o.fy:
            s.blit(f.render("|",1,[255,255,255]),[o.x,o.y+o.fy*8])

class LocalWall(local.Mob):
    def draw(o,s):
        c = o.x,o.y
        ul = o.x-o.radius,o.y-o.radius
        wh = o.radius*2,o.radius*2
        pygame.draw.rect(s,[240,220,200],[ul,wh])
...
class GameServer(GS):
    ...
    def msg_user_chat(self,message):
        ...
        if "wall" in message:
            o = self.current_player.get_object(self)
            w = self.objects.add(Wall())
            w.x,w.y = o.x,o.y
            w.name = "W"
    ...
    def filter_states(self,states,player):
        """Anything out of range, 100 pixels away, player doesn't see"""
        s = {}
        if not player.actors:
            return s
        o = player.get_object(self)
        #~ if o.name != "s":
            #~ return states
        opponents = []
        walls = []
        for k in states:
            o2 = self.objects[k]
            if isinstance(o2,MovingMob) and o2!=o:
                opponents.append(o2)
            elif isinstance(o2,Wall):
                walls.append(o2)
                opponents.append(o2)
        hide = set()
        for o2 in opponents:
